...
export function TaskCard({ issue, run }: TaskCardProps) {
  ...
  const priorityColor = issue.priority != null ? PRIORITY_COLORS[issue.priority] ?? "#9b9b9b" : null;

  return (
    <div
      ref={setNodeRef}
      style={style}
      className="group bg-white rounded-lg border border-gray-200 p-3 cursor-grab active:cursor-grabbing hover:border-gray-300 hover:shadow-sm transition-shadow"
      {...attributes}
      {...listeners}
    >
